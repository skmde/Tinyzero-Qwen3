...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        lora_rank = int(lora_config.get('rank', 0)) if optim_config is not None else 0
        enable_lora = lora_rank > 0
        use_qlora = bool(lora_config.get('use_qlora', False)) if enable_lora else False
        lora_alpha = int(lora_config.get('alpha', 16))
        lora_dropout = float(lora_config.get('dropout', 0.05))
        lora_target_modules = lora_config.get('target_modules', ['q_proj', 'k_proj', 'v_proj', 'o_proj'])

        quantization_config = None
        if use_qlora:
            from transformers import BitsAndBytesConfig
            compute_dtype_name = str(lora_config.get('bnb_4bit_compute_dtype', 'bfloat16'))
            compute_dtype = getattr(torch, compute_dtype_name)
            quantization_config = BitsAndBytesConfig(
                load_in_4bit=True,
                bnb_4bit_quant_type=str(lora_config.get('bnb_4bit_quant_type', 'nf4')),
                bnb_4bit_use_double_quant=bool(lora_config.get('bnb_4bit_use_double_quant', True)),
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if enable_gradient_checkpointing and not use_qlora:
        ...
        if self._is_actor:
            ...
            actor_parameters = actor_module_fsdp.parameters() if not enable_lora else \
                [param for param in actor_module_fsdp.parameters() if param.requires_grad]
            actor_optimizer = optim.AdamW(actor_parameters,
                                          lr=optim_config.lr,
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                try:
                    ...
                    raise ImportError('flash_attn is required when use_remove_padding=True. Install flash-attn or set use_remove_padding=False.') from exc
